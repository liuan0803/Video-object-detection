...
class VIDDemo(object):
    ...
    #               'whale', 'zebra']
    CATEGORIES = ['__background__',  # always index 0
                    'boat1', 'plane', 'ragged', 'quadrocopter']
    ...
    def __init__(
        ...
        ############ add new save txt path ##########
        self.savetxt_path = savetxt_path
        ...
        for frame_id in tqdm(range(frame_seg_len)):
            ...
            if self.method == "base":
                ...
                if self.method == "dff":
                    ...
                    ref_filename = pattern % ref_id # 因为前面定义的时候是格式化定义
                    img_ref = cv2.imread(img_dir % ref_filename) # 因为前面定义的时候是格式化定义
                ...
                # self.run_on_image_savetxt_drawbbox(original_image, imgname_cur,
                #                                                       img_cur)
                images_with_boxes.append(image_with_boxes)
            else:
    ...
    def savetxt_and_drawbbox(self, image, imgname_cur, top_predictions):
        '''
        savetxt_and_drawbbox 检测单张图片并将bbox画在image上保存txt
        :param image: resize前原图
        :param imgname_cur: 当前处理图片的原始名称
        :param top_predictions: 根据阈值筛选的检测框及附属信息
        :return:
        '''
        ...
        with open(os.path.join(self.savetxt_path, imgname_cur + '.txt'), 'w') as f:
            line = ''
            for box, color, score, label in zip(boxes, colors, scores, labels):
                xmin, ymin = [int(val) for val in box[:2]]
                xmax, ymax = [int(val) for val in box[2:]]
                s = template.format(label, score)
                line += txt_text.format(label, score, xmin, ymin, xmax, ymax)
                cv2.rectangle(
                    image, (xmin, ymin), (xmax,ymax), tuple(color), 2
                )
                cv2.putText(
                    image, s, (xmin, ymin), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2
                )
            f.write(line)
        f.close()
        cv2.imwrite(self.output_folder + '/' +  imgname_cur + '.jpg', image)

    def run_on_image_savetxt_drawbbox(self, image, imgname_cur, infos=None):
        """
        Arguments:
            image
            imgname_cur
        ...
        Returns:
        """
        # print("*******current inferencing number now is : ", imgname_cur)
        predictions = self.compute_prediction(image, infos)
        top_predictions = self.select_top_predictions(predictions)

        result = image.copy()
        self.savetxt_and_drawbbox(result, imgname_cur, top_predictions)
    ...
    def run_on_image_folder_savetxt_drawbbox(self, image_folder, suffix='.JPEG'):
        image_names = glob.glob(image_folder + '/*' + suffix)
        image_names = sorted(image_names)
        ...
        images_with_boxes = []
        # begin_num = int((image_names[0].split("/")[-1]).split('.')[0])
        ...
        for frame_id in tqdm(range(frame_seg_len)):
            ...
            if self.method == "base":
                ...
                if self.method == "dff":
                    ...
                    img_refs = []
                    if self.method == "fgfa":
                        max_offset = self.cfg.MODEL.VID.FGFA.MAX_OFFSET # FGFA.MAX_OFFSET默认为9
                    else:
                        max_offset = self.cfg.MODEL.VID.RDN.MAX_OFFSET # RDN.MAX_OFFSET默认为0
                    ref_id = min(frame_seg_len - 1, frame_id + max_offset)
                    ref_filename = pattern % ref_id # 因为前面定义的时候是格式化定义
                    # temp = img_dir % ref_filename
                    img_ref = cv2.imread(img_dir % ref_filename) # 因为前面定义的时候是格式化定义
                    img_ref = self.perform_transform(img_ref)
                    img_refs.append(img_ref)
